...
def process_json_files():
	try:
		...
		for filename in os.listdir(common.PINUT_FEEDBACK_FILE_PATH):
			...
			with open(common.PINUT_FEEDABCK_FILE_PATH+"/"+filename, 'r+') as f_in:
			...
			mongo_db_obj.mongo_ensure_index(cust_name, "pinut_user_intro_data", ["cust_name","lid","date","pinut_mac"])
                    for key,value in data.iteritems():
                        ...
                        if str(cust_name) == "woobus":
                            ...
                            staff = int(value['staff'])
                            snacks = int(value['snacks'])
                            cleanliness = int(value['cleanliness'])
                            in_app_entertainment = int(value['in_app_entertainment'])
                            bus_tracking = int(value['bus_tracking'])
                            punctuality = int(value['punctuality'])
                            overall_experience = int(value['overall_experience'])
                            ...
                            #row={"comment": "Good ", "pinut_experience": 5, "name": "preety", "cl_mac": "5c:51:88:07:8f:15", "email_id": "pranetazvision@yahoo.co.in", "phone": "7022895195", "ride_experience": 5}
                            pinut_experience = int(value['pinut_experience'])
                            ride_experience = int(value['ride_experience'])
